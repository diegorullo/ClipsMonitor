 ...
 */
public class AssistedLivingAgentMap extends MonitorMap implements Observer {
    ...
    //[MP] KCell: Adeguamento del metodo in base agli slot previsti dal progetto attuale
    private void updateKCells() throws CLIPSError{
        console.debug("Aggiornamento mappa dell'agente in corso...");

        String[][] cellFacts = core.findAllFacts("AGENT", AssistedLivingFacts.KCell.factName(), "TRUE", AssistedLivingFacts.KCell.slotsArray());

        for (String[] fact : cellFacts) {

            // Nei fatti si conta partendo da 1, nella matrice no, quindi sottraiamo 1.
            int c = new Integer(fact[AssistedLivingFacts.KCell.POSC.index()]) - 1;
            int r = new Integer(fact[AssistedLivingFacts.KCell.POSR.index()]) - 1;
            String contains = fact[AssistedLivingFacts.KCell.CONTAINS.index()];
